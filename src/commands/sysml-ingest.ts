...
export default class SysmlIngest extends Command {
  static description = "Reverse engineer a codebase into SysML v2 models";

  static examples = [
    "<%= config.bin %> sysml-ingest",
    "<%= config.bin %> sysml-ingest --cycle 0",
    "<%= config.bin %> sysml-ingest --cycle 1",
    "<%= config.bin %> sysml-ingest --cycle 3 --model opus",
    "<%= config.bin %> sysml-ingest --purge",
    "<%= config.bin %> sysml-ingest --skip-cycle0",
    "<%= config.bin %> sysml-ingest -v",
  ];
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(SysmlIngest);
  ...
  private async runSingleCycleTurn(
    ...
    const maxTurnIterations = Math.min(flags["max-iterations"], 10);
